...
<HTML>
	...
	<body onload="Initialize();" ms_positioning="GridLayout">
											...
											<tr>
												<td>
													<button class="iCWebControlsII" id="Update" title="Update" onclick="javascript:SaveContent()"
														name="Update" type="button"><%=Update%></button>&nbsp;<button class="iCWebControlsII" id="Cancel" title="Cancel" onclick="window.history.go(-1);"
		...
		<form name="Editor" onsubmit="submitForm();" action="PublisherEdit.aspx" method="post">
			...
			<input id="HtmlMode" type="hidden" value="<%=HtmlMode%>" name="HtmlMode" >
			<input id="Alert" type="hidden" value="<%=Alert%>" name="HtmlMode" >
		</form>
		<form id="PreWriter" runat="server">
			<asp:TextBox id="DesktopText" runat="server" textmode="multiline" rows="0" width="0" columns="0" style="height:0;Width:0"></asp:TextBox>
		</form>
		<script>
		</script>
	</body>
