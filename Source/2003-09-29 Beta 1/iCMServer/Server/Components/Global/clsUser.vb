Public Class clsUser
    ...
    Public Function GetRoles(ByVal email As String) As String
        ...
        If Not oDO.GetDataSet("usr_users", "usr_email = '" & email & "' AND usr_deleted = 0", "", sError, ED, EC, dsUsers) Then
            System.Diagnostics.Debug.WriteLine(sError)
        End If
        UserId = dsUsers.Tables(0).Rows(0)("usr_id")
        If Not oDO.GetDataSet("uro_userroles", "usr_id = " & UserId & " AND uro_deleted = 0", "", sError, ED, EC, dsUserRoles) Then
            System.Diagnostics.Debug.WriteLine(sError)
        End If
        If dsUserRoles.Tables.Count > 0 Then
            If dsUserRoles.Tables(0).Rows.Count > 0 Then
                ...
                If Not oDO.GetDataSet("rol_roles", "rol_id = " & sWhere & " AND rol_deleted = 0", "", sError, ED, EC, dsRoles) Then
                    System.Diagnostics.Debug.WriteLine(sError)
                End If
