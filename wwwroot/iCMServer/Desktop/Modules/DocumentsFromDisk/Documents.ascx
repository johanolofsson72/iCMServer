...
<%@ Control Language="vb" AutoEventWireup="false" Codebehind="Documents.ascx.vb" Inherits="iConsulting.iCMServer.Modules.DocumentsFromDisk.Documents" targetSchema="http://schemas.microsoft.com/intellisense/ie5" %>
...
<script language="javascript">
	// WebFX Config...
	webFXTreeConfig.rootIcon		= "f.png";
	webFXTreeConfig.openRootIcon	= "o.png";
	webFXTreeConfig.folderIcon		= "f.png";
	webFXTreeConfig.openFolderIcon	= "o.png";
	webFXTreeConfig.fileIcon		= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/file.png";
	webFXTreeConfig.lMinusIcon		= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/Lminus.png";
	webFXTreeConfig.lPlusIcon		= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/Lplus.png";
	webFXTreeConfig.tMinusIcon		= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/Tminus.png";
	webFXTreeConfig.tPlusIcon		= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/Tplus.png";
	webFXTreeConfig.iIcon			= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/I.png";
	webFXTreeConfig.lIcon			= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/L.png";
	webFXTreeConfig.tIcon			= "Desktop/Modules/DocumentsFromDisk/XLoadTree/images/xp/T.png";
	var <%=TreeName%> = new WebFXLoadTree("<%=ModTitle%>", "Desktop/Modules/DocumentsFromDisk/DocumentsEngine.aspx?sID=r,<%=ModId%>,<%=System.Web.HttpContext.Current.Server.UrlEncode(BoundedDirectory.Replace("\","/"))%>", "javascript:GD<%=ModId%>('<%=BoundedDirectory.Replace("\", "/")%>')", "explorer", "f.png","o.png");
...
<script language=javascript>
	...
	var Object_ID<%=ModId%>="Object_ID";
	var SelectedSubItem<%=ModId%>="SelectedSubItem";
	var DeselectedSubItem<%=ModId%>="DeselectedSubItem";
	var Report<%=ModId%>="";
	var CurrentFolder<%=ModId%>="CurrentFolder";
	...
	function AddFolder<%=ModId%>(){
		if(!Object_Focus<%=ModId%>==""){
			...
			if(title==null){
				...
				document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsFolderEdit.aspx?Mode=ADDFOLDER&CatId="+CurrentFolder<%=ModId%>+"&ModId="+"<%=ModId%>&Title="+title+"&PagId="+<%=PagId%>;
			}
		}
	}
	function EditFolder<%=ModId%>(){
		if((Object_Root<%=ModId%>==false) && (!Object_Focus<%=ModId%>=="")){
			...
			if(title==null){
				...
				document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsFolderEdit.aspx?Mode=EDITFOLDER&CatId="+CurrentFolder<%=ModId%>+"&ModId="+"<%=ModId%>&Title="+title+"&PagId="+<%=PagId%>;
			}
		}
	}
	function DeleteFolder<%=ModId%>(){
		if((Object_Root<%=ModId%>==false) && (!Object_Focus<%=ModId%>=="")){
			if(confirm("<%=DeleteFolderConfirm%>"+Object_Focus<%=ModId%>.text+" ?")){
				document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsFolderEdit.aspx?Mode=DELETEFOLDER&CatId="+CurrentFolder<%=ModId%>+"&ModId="+"<%=ModId%>"+"&PagId="+<%=PagId%>;
			}
		}
	}
	function EditFile<%=ModId%>(){
		document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsEdit.aspx?CatId="+CurrentFolder<%=ModId%>+"&ModId="+"<%=ModId%>"+"&PageId="+<%=PagId%>;
	}
	function UploadFile<%=ModId%>(){
		document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsEdit.aspx?CatId="+CurrentFolder<%=ModId%>+"&ModId="+"<%=ModId%>"+"&PageId="+<%=PagId%>;
	}
	function UploadFileSimple<%=ModId%>(){
		window.open('Desktop/Modules/DocumentsFromDisk/DocumentsEditSimple.aspx?ModId='+'<%=ModId%>'+'&PageId='+<%=PagId%>,'DocumentsEdit','Height=350,Width=500');
	}
	function GetDocuments<%=ModId%>(Directory){
		try{
			//alert(Directory);
			SetCurrentFolder<%=ModId%>(Directory);
			var d = new Date()
			var sTime = d.getHours() + d.getMinutes() + d.getSeconds()
			document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsGetFiles.aspx?Directory="+escape(Directory)+"&ModId="+"<%=ModId%>&H="+"<%=HasEditAuth%>"+"&Time="+sTime;
		}
		catch(e){
		
		}
	}
	function GD<%=ModId%>(Directory){
		GetDocuments<%=ModId%>(Directory);
	}
	function Search<%=ModId%>(){
		var Search = document.getElementById("txtSearchBox").value;
		var d = new Date()
		var sTime = d.getHours() + d.getMinutes() + d.getSeconds()
		var Directory = "<%=BoundedDirectory%>"
		<%If SimpleViewState Then%>
			//document.getElementById("SimpleFilesHolder<%=ModId%>").src="Desktop/Modules/Documents/DocumentsGetFiles.aspx?CatId=0&ModId="+"<%=ModId%>&H="+"<%=HasEditAuth%>"+"&Time="+sTime+"&Search="+Search+"&State=1";
		<%Else%>
			document.getElementById("FileContainer<%=ModId%>").src="Desktop/Modules/DocumentsFromDisk/DocumentsGetFiles.aspx?Directory="+escape(Directory)+"&ModId="+"<%=ModId%>&H="+"<%=HasEditAuth%>"+"&Time="+sTime+"&Search="+Search;
		<%End If%>
	}	
...
<div id="Minimizer" runat="server">
					...
					<TR>
						...
						<TD <%If Not SimpleViewState Then%>bgColor="#ffffff"<%End If%> align="left">
							<table width="100%" border="0" cellSpacing="0" cellPadding="0">
								<tr>
									<%If HasEditAuth Then%>
										<td align="left"><IMG id=btnRefresh style="CURSOR: hand" onclick=Refresh<%=ModId%>() alt="<%=RefreshAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/Refresh.gif" ></td>
										<td align="left"><div nowrap id="lblRefresh" style="CURSOR: hand" class="SubSubHead" onclick="Refresh<%=ModId%>()">&nbsp;<%=RefreshAltText%>&nbsp;&nbsp;</div></td>
										<td align="left"><asp:TextBox ID="txtDir" Runat="server"></asp:TextBox>&nbsp;<asp:Button ID="btnDir" Runat="server"></asp:Button></td>
									<%End If%>
									<%If Not SimpleViewState Then%>
										<%If HasEditAuth And 1 = 2 Then%>
											<td align="left"><IMG id=btnAddFolder style="CURSOR: hand" onclick=AddFolder<%=ModId%>() alt="<%=AddFolderAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/AddFolder.gif" ></td>
											...
											<td align="left"><IMG id=btnEditFolder style="CURSOR: hand" onclick=EditFolder<%=ModId%>() alt="<%=EditFolderAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/EditFolder.gif" ></td>
											...
											<td align="left"><IMG id=btnDeleteFolder style="CURSOR: hand" onclick=DeleteFolder<%=ModId%>() alt="<%=DeleteFolderAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/DeleteFolder.gif" ></td>
											...
											<td align="left"><IMG id=btnUploadFile style="CURSOR: hand" onclick=UploadFile<%=ModId%>() alt="<%=UploadFileAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/UploadFile.gif" ></td>
									...
									<%Else%>
										<%If HasEditAuth And 1 = 2 Then%>
											<td align="left"><IMG id=btnUploadFile2 style="CURSOR: hand" onclick=UploadFileSimple<%=ModId%>() alt="<%=UploadFileAltText%>" src="Desktop/Modules/DocumentsFromDisk/Images/UploadFile.gif" ></td>
		...
		<tr>
			...
			<td <%If Not SimpleViewState Then%>bgColor="#ffffff" style="BORDER-RIGHT: black 1px solid;BORDER-LEFT: black 1px solid;BORDER-BOTTOM: black 1px solid;"<%End If%>>
				<TABLE height="21" cellSpacing="0" cellPadding="1" width="100%" align="left" border="0">
					<tr>
						<%If Not SimpleViewState Then%>
						<TD>&nbsp;<span style="WIDTH: 2px"></span></TD>
						<TD><input type="text" value="" id="txtSearchBox" onkeydown="KeyDown()" style="width:160px;height:19px" class="label"></TD>
						<TD>&nbsp;</TD>
						...
						<%Else%>
						<TD>&nbsp;<span style="WIDTH: 2px"></span></TD>
						<TD><asp:TextBox ID="txtSearchBox2" CssClass="label" Height="19" MaxLength="50" Width="160" onkeydown="KeyDown()" style="width:160px;height:19px" Runat="server"></asp:TextBox></TD>
						<TD>&nbsp;</TD>
						<TD><asp:Button style="height:19px;cursor:hand" Runat="server" Text="Sök" ID="btnSearchButton2" CssClass="iCWebControlsII"></asp:Button></TD>
						<TD width="90%">&nbsp;</TD>
						<%End If%>
					</tr>
				</TABLE>
	...
	<script>
		try{
			//GetDocuments<%=ModId%>('po');
			GetDocuments<%=ModId%>('<%=BoundedDirectory.Replace("\", "/")%>');
		}
		catch(e){
		...
		function mySubmit()
		{
			try{
				var e1;
				<%If SimpleViewState Then%>
					e1 = document.getElementById(SimpleButtonId);
				<%Else%>
					e1 = document.getElementById("btnSearchButton");
				<%End If%>
				e1.focus();
			}
			catch(e){
				//alert('fel i mySubmit();')
			}
		}
		try{
			var e1;
			<%If SimpleViewState Then%>
				e1 = document.getElementById(SimpleBoxId);
			<%Else%>
				e1 = document.getElementById("txtSearchBox");
			<%End If%>
			e1.focus();
		}
		catch(e){
